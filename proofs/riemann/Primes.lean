...
/--
Prime embedding: ψ_p : ⟨p⟩ → N_all
This is just the colimit inclusion for a prime p.
-/
def prime_embedding (p : ℕ) (h : is_prime p) : GenObj.nat p → NAllObj :=
  include p

/-- Notation for prime embedding -/
notation "ψ_" p => prime_embedding p
...
/--
A morphism to ⟨p⟩ is atomic if it cannot factor through
a proper intermediate object (except ∅ and 𝟙).
-/
def is_atomic_morphism (p : ℕ) (h_prime : is_prime p)
    (f : GenMorphism 𝟙 (GenObj.nat p)) : Prop :=
  ∀ (n : ℕ) (g : GenMorphism 𝟙 (GenObj.nat n))
    (k : GenMorphism (GenObj.nat n) (GenObj.nat p)),
    f = GenMorphism.comp g k →
    n = 1 ∨ n = p

/-- **Theorem Prime.1**: Primes are categorical atoms -/
theorem primes_are_atoms (p : ℕ) (h_prime : is_prime p) :
  is_atomic_morphism p h_prime (GenMorphism.instantiation p) := by
  intro n g k h_factor
  -- If ι_p factors as ι_p = k ∘ ι_n, then n | p
  -- k : n → p means n | p by the morphism criterion
  -- Since p is prime, either n = 1 or n = p
  have ⟨hp_gt, hp_prime⟩ := h_prime
  -- k is a morphism n → p, so n | p
  have h_div : n ∣ p := by
    cases k
...
structure PrimeFactorization where
  factors : List (ℕ × ℕ)
  all_prime : ∀ (p e) ∈ factors, is_prime p
  positive_exponents : ∀ (p e) ∈ factors, e > 0
  sorted : factors.Sorted (fun a b => a.1 < b.1)

/-- Every n > 1 has a prime factorization -/
axiom prime_factorization_exists (n : ℕ) (h : n > 1) :
  ∃ (pf : PrimeFactorization), n = pf.factors.foldl (fun acc (p, e) => acc * p ^ e) 1

/-- Prime factorization is unique -/
axiom prime_factorization_unique (n : ℕ) (h : n > 1)
...
def has_unique_prime_factorization (x : NAllObj) : Prop :=
  ∃ (n : ℕ) (h : n > 1) (pf : PrimeFactorization) (y : GenObj.nat n),
    x = include n y ∧
...
theorem fundamental_theorem_arithmetic :
  ∀ (x : NAllObj),
    (∃ (n : ℕ) (h : n > 1) (y : GenObj.nat n), x = include n y) →
...
/-- Prime power embedding: ψ_{p^k} : ⟨p^k⟩ → N_all -/
def prime_power_embedding (p k : ℕ) : GenObj.nat (p ^ k) → NAllObj :=
  include (p ^ k)
...
theorem nall_generated_by_primes
    (f g : NAllObj → NAllObj)
    (h_agree_on_primes : ∀ (p : ℕ) (h_prime : is_prime p) (x : GenObj.nat p),
  ...
  case mk =>
    ...
    have h_2_prime : is_prime 2 := by
      ...
      · intro n hn
        -- n | 2 implies n ∈ {1, 2}
        have : n ≤ 2 := Nat.le_of_dvd (by omega) hn
        omega  -- n = 1 or n = 2
...
theorem prime_divisibility_irreducible (p n : ℕ)
    (h_prime : is_prime p) (h_div : p ∣ n) :
  ∀ (k : ℕ), (∃ m, n = k * m) → (k ∣ p ∨ p ∣ k) → (k = 1 ∨ k = p) := by
  intro k ⟨m, hm⟩ h_div_rel
...
theorem prime_morphism_monic (p : ℕ) (h_prime : is_prime p) :
  ∀ (X : GenObj) (f g : GenMorphism X (GenObj.nat p)),
    (∀ (m : GenMorphism (GenObj.nat p) (GenObj.nat p)),
      GenMorphism.comp f m = GenMorphism.comp g m) →
    f = g := by
  intro X f g h_comp
...
theorem prime_endomorphisms_simple (p : ℕ) (h_prime : is_prime p)
    (f : GenMorphism (GenObj.nat p) (GenObj.nat p)) :
  f = GenMorphism.id_nat p ∨
  ∃ (k : ℕ) (h : ∃ m, p * m = k), f = GenMorphism.divisibility p k h := by
  cases f
  case id_nat =>
    left
    rfl
...
The prime structure directly influences ζ_gen through the Euler product.
This will be developed further in Gen.ZetaProperties.
-/

/-- Placeholder: Primes determine ζ_gen through Euler product -/
